      ...
      try {
        ...
        const completion = await openai.chat.completions.create({
          ...
          messages: [
            {
              role: "system",
              content: `You are an expert at creating detailed image generation prompts that capture the essence of music and create personalized scenes.
Your prompts should:
1. Create a scene that represents the person's music taste in a personal space
2. Include visual elements that represent their top tracks (like posters, album art, or music-themed decorations)
3. Show a person (or their silhouette) enjoying the music in a relatable setting
4. Use lighting and atmosphere that matches the mood of their music
5. Be highly detailed and specific about the environment and visual elements
6. Be suitable for DALL-E image generation
7. Be 2-3 sentences long`
            },
            {
              role: "user",
              content: `Create a detailed prompt for DALL-E to generate an image that represents this person's music taste in a personal space. Their top tracks are:\n${tracks.map((track: any, i: number) => `${i + 1}. ${track.name} by ${track.artist}`).join('\n')}\n\nConsider the overall mood and style of these tracks when creating the scene. The image should feel personal and relatable, like a snapshot of someone's space that reflects their music taste.`
            }
          ],
          temperature: 0.7,
          max_tokens: 300
        ...
        // Send the image to Discord with a more personalized message
        await sendDiscordFollowup(applicationId, interactionToken, '', [{
          title: `🎨 ${username}'s Music Space`,
          description: `*A visual representation of your music taste*\n\n**Scene Description:** ${imagePrompt}\n\n*Your top tracks are featured in this personalized space, creating a unique atmosphere that reflects your musical journey.*`,
          image: { url: imageUrl || '' },
          color: 0x1DB954,
